# POC: Finding the best 2-component mixture fit for input data with custom distribution list
...
# 3. Fit custom 2-component mixtures
cat("\nFitting custom selection of 2-component mixtures using fit_all_2m()...\n")
# Using a subset of distributions to make the POC run reasonably fast
custom_dists <- c("lognormal_2p", "weibull_2p", "gamma_2p", "pareto_2p")
all_fits <- fit_all_2m(sample_data, dist_names = custom_dists, criterion = "BIC")
...
for (i in 1:length(all_fits)) {
  fit <- all_fits[[i]]
  cat(sprintf("%-35s | %-12.2f | %-12.2f | %-15.2f\n", 
...
output_dir <- "outputs"
output_file <- "poc_best_fit_2m_custom"
...
plot_cdf_comparison(
  ...
  title = sprintf("Best Custom 2-Component Mixture Fit (Data: %s, Best: %s)", 
