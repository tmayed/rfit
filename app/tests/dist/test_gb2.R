...
test_data <- gb2_rand(1000, true_params)

# Test gb2_log_likelihood
cat("Testing gb2_log_likelihood...\n")
ll <- gb2_log_likelihood(test_data, 2, 10, 1.5, 2.5)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# Test gb2_fit
cat("Testing gb2_fit...\n")
fit <- gb2_fit(test_data)
...
# Test gb2_fit_truncated
cat("Testing gb2_fit_truncated...\n")
trunc_data <- test_data[test_data > 2 & test_data < 50]
fit_trunc <- gb2_fit_truncated(trunc_data, lower = 2, upper = 50)
cat(sprintf("  a: %.4f\n", fit_trunc$a))
cat(sprintf("  b: %.4f\n", fit_trunc$b))
...
# Test PDF/CDF
cat("Testing gb2_pdf/cdf...\n")
x <- seq(1, 50, length.out = 6)
dens <- gb2_pdf(x, fit)
cdf_vals <- gb2_cdf(x, fit)
...
# Test Quantile
cat("Testing gb2_quantile...\n")
p <- seq(0.1, 0.9, length.out = 10)
quantiles <- gb2_quantile(p, fit)
...
stopifnot(all(abs(gb2_cdf(isf, fit) - (1 - p)) < 1e-10))

# Test Logs
cat("Testing Log PDF/CDF/SF...\n")
lpdf <- gb2_logpdf(x, fit)
lcdf <- gb2_logcdf(x, fit)
lsf <- gb2_logsf(x, fit)
stopifnot(all(abs(lpdf - log(gb2_pdf(x, fit))) < 1e-10))
stopifnot(all(abs(lcdf - log(gb2_cdf(x, fit))) < 1e-10))
stopifnot(all(abs(lsf - log(gb2_sf(x, fit))) < 1e-10))

# Test Stats
cat("Testing Stats (Mean, Var, Std, Skew, Kurtosis, Median)...\n")
mean_val <- gb2_mean(fit)
var_val <- gb2_var(fit)
std_val <- gb2_std(fit)
skew_val <- gb2_skew(fit)
kurt_val <- gb2_kurtosis(fit)
med_val <- gb2_median(fit)
...
stopifnot(abs(med_val - gb2_quantile(0.5, fit)) < 1e-10)

# Test Moment
cat("Testing gb2_moment...\n")
m1 <- gb2_moment(1, fit)
m2 <- gb2_moment(2, fit)
...
# Test Interval
cat("Testing gb2_interval...\n")
int <- gb2_interval(0.90, fit)
cat(sprintf("  90%% Interval: [%.4f, %.4f]\n", int[1], int[2]))
stopifnot(abs(gb2_cdf(int[2], fit) - gb2_cdf(int[1], fit) - 0.90) < 1e-10)

# Test Entropy/Expect
cat("Testing gb2_entropy/expect...\n")
ent <- gb2_entropy(fit)
e_x <- gb2_expect(function(x) x, fit)
