...
ll <- pareto_log_likelihood(test_data, shape_true, scale_true)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# Test fit
cat("Testing pareto_fit...\n")
fit <- pareto_fit(test_data)
...
fit_t <- pareto_fit_truncated(trunc_data, lower = lower_bound, upper = upper_bound)
...
p <- c(0.025, 0.5, 0.975)
qs <- pareto_quantile(p, fit)
cat(sprintf("  Quantiles: %.4f, %.4f, %.4f\n", qs[1], qs[2], qs[3]))
stopifnot(all(diff(qs) > 0))

# Test ISF
cat("Testing pareto_isf...\n")
isf <- pareto_isf(p, fit)
cat(sprintf("  ISF: %s\n", paste(sprintf("%.4f", isf), collapse = ", ")))
stopifnot(all(abs(pareto_cdf(isf, fit) - (1 - p)) < 1e-10))

# Test Logs
cat("Testing Log PDF/CDF/SF...\n")
lpdf <- pareto_logpdf(x, fit)
lcdf <- pareto_logcdf(x, fit)
lsf <- pareto_logsf(x, fit)
stopifnot(all(abs(lpdf - log(pareto_pdf(x, fit))) < 1e-10))
# CDF can be 0 at scale, so use is.finite for log
stopifnot(all(abs(lcdf[cdf_vals > 0] - log(cdf_vals[cdf_vals > 0])) < 1e-10))
stopifnot(all(abs(lsf - log(pareto_sf(x, fit))) < 1e-10))
...
var_val <- pareto_var(fit)
std_val <- pareto_std(fit)
skew_val <- pareto_skew(fit)
kurt_val <- pareto_kurtosis(fit)
med_val <- pareto_median(fit)
...
# Test Moment
cat("Testing pareto_moment...\n")
m1 <- pareto_moment(1, fit)
m2 <- pareto_moment(2, fit)
cat(sprintf("  1st Moment: %.4f, 2nd Moment: %.4f\n", m1, m2))
stopifnot(abs(m1 - mean_val) < 1e-10)
stopifnot(abs(m2 - (var_val + mean_val^2)) < 1e-10)

# Test Interval
cat("Testing pareto_interval...\n")
int <- pareto_interval(0.95, fit)
cat(sprintf("  95%% Interval: [%.4f, %.4f]\n", int[1], int[2]))
stopifnot(abs(pareto_cdf(int[2], fit) - pareto_cdf(int[1], fit) - 0.95) < 1e-10)

# Test Entropy
cat("Testing pareto_entropy...\n")
ent <- pareto_entropy(fit)
cat(sprintf("  Entropy: %.4f\n", ent))
stopifnot(is.finite(ent))

# Test Expect
cat("Testing pareto_expect...\n")
e_x <- pareto_expect(function(x) x, fit)
cat(sprintf("  E[x]: %.4f\n", e_x))
