...
cat("Testing weibull_2p_log_likelihood...\n")
ll <- weibull_2p_log_likelihood(test_data, 2, 5)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# 2. Test fit
cat("Testing weibull_2p_fit...\n")
fit <- weibull_2p_fit(test_data)
...
# 3. Test PDF / LogPDF
cat("Testing weibull_2p_pdf/logpdf...\n")
x <- seq(0.1, 10, length.out = 5)
dens <- weibull_2p_pdf(x, fit)
log_dens <- weibull_2p_logpdf(x, fit)
...
cat("Testing weibull_2p_cdf/sf/logcdf/logsf...\n")
cdf_vals <- weibull_2p_cdf(x, fit)
sf_vals <- weibull_2p_sf(x, fit)
log_cdf_vals <- weibull_2p_logcdf(x, fit)
log_sf_vals <- weibull_2p_logsf(x, fit)
...
# 5. Test Quantile / ISF
cat("Testing weibull_2p_quantile/isf...\n")
p <- c(0.025, 0.5, 0.975)
qs <- weibull_2p_quantile(p, fit)
isfs <- weibull_2p_isf(1 - p, fit)
...
m <- weibull_2p_mean(fit)
v <- weibull_2p_var(fit)
s <- weibull_2p_std(fit)
sk <- weibull_2p_skew(fit)
kt <- weibull_2p_kurtosis(fit)
...
med <- weibull_2p_median(fit)
int <- weibull_2p_interval(0.95, fit)
m2 <- weibull_2p_moment(2, fit)
cat(sprintf("  Median: %.4f\n", med))
cat(sprintf("  95%% Interval: [%.4f, %.4f]\n", int[1], int[2]))
stopifnot(all.equal(med, weibull_2p_quantile(0.5, fit)))
stopifnot(all.equal(m2, v + m^2))

# 8. Test Entropy / Expect
cat("Testing entropy/expect...\n")
ent <- weibull_2p_entropy(fit)
exp_x <- weibull_2p_expect(function(x) x, fit)
