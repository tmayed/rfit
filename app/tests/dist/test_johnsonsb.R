...
cat("Testing johnsonsb_4p_rand...\n")
test_data <- johnsonsb_4p_rand(1000, fit_true)
stopifnot(all(test_data >= 0 & test_data <= 10))
cat(sprintf("  Mean of sample: %.4f\n", mean(test_data)))

# 2. Test Fit
cat("Testing johnsonsb_4p_fit...\n")
fit_hat <- johnsonsb_4p_fit(test_data)
...
# 3. Test PDF / LogPDF
cat("Testing johnsonsb_4p_pdf/logpdf...\n")
x <- seq(1, 9, length.out = 5)
dens <- johnsonsb_4p_pdf(x, fit_hat)
log_dens <- johnsonsb_4p_logpdf(x, fit_hat)
stopifnot(all(dens > 0))
stopifnot(all.equal(log(dens), log_dens))

# 4. Test CDF / SF / LogCDF / LogSF
cat("Testing johnsonsb_4p_cdf/sf/logcdf/logsf...\n")
cdf_vals <- johnsonsb_4p_cdf(x, fit_hat)
sf_vals <- johnsonsb_4p_sf(x, fit_hat)
stopifnot(all(cdf_vals >= 0 & cdf_vals <= 1))
stopifnot(all.equal(cdf_vals + sf_vals, rep(1, length(x))))
stopifnot(all.equal(log(cdf_vals), johnsonsb_4p_logcdf(x, fit_hat)))
stopifnot(all.equal(log(sf_vals), johnsonsb_4p_logsf(x, fit_hat)))

# 5. Test Quantile / ISF
cat("Testing johnsonsb_4p_quantile/isf...\n")
p <- c(0.25, 0.5, 0.75)
qs <- johnsonsb_4p_quantile(p, fit_hat)
isfs <- johnsonsb_4p_isf(1 - p, fit_hat)
stopifnot(all.equal(qs, isfs))
stopifnot(all.equal(johnsonsb_4p_cdf(qs, fit_hat), p))

# 6. Test Mean / Var / Std
cat("Testing moments/statistics...\n")
m <- johnsonsb_4p_mean(fit_hat)
v <- johnsonsb_4p_var(fit_hat)
s <- johnsonsb_4p_std(fit_hat)
...
med <- johnsonsb_4p_median(fit_hat)
int <- johnsonsb_4p_interval(0.95, fit_hat)
m1 <- johnsonsb_4p_moment(1, fit_hat)
stopifnot(all.equal(med, johnsonsb_4p_quantile(0.5, fit_hat)))
stopifnot(all.equal(m1, m, tolerance = 1e-5))

# 8. Test Entropy / Expect
cat("Testing entropy/expect...\n")
ent <- johnsonsb_4p_entropy(fit_hat)
exp_x <- johnsonsb_4p_expect(function(x) x, fit_hat)
