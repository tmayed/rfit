...
cat("Testing bradford_1p_rand...\n")
test_data <- bradford_1p_rand(1000, fit_true)
stopifnot(all(test_data >= 0 & test_data <= 10))
cat(sprintf("  Mean of sample: %.4f\n", mean(test_data)))

# 2. Test Fit
cat("Testing bradford_1p_fit...\n")
fit_hat <- bradford_1p_fit(test_data)
...
# 3. Test PDF / LogPDF
cat("Testing bradford_1p_pdf/logpdf...\n")
x <- seq(1, 9, length.out = 5)
dens <- bradford_1p_pdf(x, fit_hat)
log_dens <- bradford_1p_logpdf(x, fit_hat)
stopifnot(all(dens > 0))
stopifnot(all.equal(log(dens), log_dens))

# 4. Test CDF / SF / LogCDF / LogSF
cat("Testing bradford_1p_cdf/sf/logcdf/logsf...\n")
cdf_vals <- bradford_1p_cdf(x, fit_hat)
sf_vals <- bradford_1p_sf(x, fit_hat)
stopifnot(all(cdf_vals >= 0 & cdf_vals <= 1))
stopifnot(all.equal(cdf_vals + sf_vals, rep(1, length(x))))
stopifnot(all.equal(log(cdf_vals), bradford_1p_logcdf(x, fit_hat)))
stopifnot(all.equal(log(sf_vals), bradford_1p_logsf(x, fit_hat)))

# 5. Test Quantile / ISF
cat("Testing bradford_1p_quantile/isf...\n")
p <- c(0.25, 0.5, 0.75)
qs <- bradford_1p_quantile(p, fit_hat)
isfs <- bradford_1p_isf(1 - p, fit_hat)
stopifnot(all.equal(qs, isfs))
stopifnot(all.equal(bradford_1p_cdf(qs, fit_hat), p))

# 6. Test Mean / Var / Std / Skew / Kurtosis
cat("Testing moments/statistics...\n")
m <- bradford_1p_mean(fit_hat)
v <- bradford_1p_var(fit_hat)
s <- bradford_1p_std(fit_hat)
sk <- bradford_1p_skew(fit_hat)
kt <- bradford_1p_kurtosis(fit_hat)
...
med <- bradford_1p_median(fit_hat)
int <- bradford_1p_interval(0.95, fit_hat)
m1 <- bradford_1p_moment(1, fit_hat)
stopifnot(all.equal(med, bradford_1p_quantile(0.5, fit_hat)))
stopifnot(all.equal(m1, m))

# 8. Test Entropy / Expect
cat("Testing entropy/expect...\n")
ent <- bradford_1p_entropy(fit_hat)
exp_x <- bradford_1p_expect(function(x) x, fit_hat)
