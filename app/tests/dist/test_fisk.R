...
cat("Testing fisk_log_likelihood...\n")
ll <- fisk_log_likelihood(test_data, test_scale, test_shape)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# 2. Test fit
cat("Testing fisk_fit...\n")
fit <- fisk_fit(test_data)
...
# 3. Test PDF / LogPDF
cat("Testing fisk_pdf/logpdf...\n")
x <- seq(0.1, 10, length.out = 5)
dens <- fisk_pdf(x, fit)
log_dens <- fisk_logpdf(x, fit)
...
cat("Testing fisk_cdf/sf/logcdf/logsf...\n")
cdf_vals <- fisk_cdf(x, fit)
sf_vals <- fisk_sf(x, fit)
log_cdf_vals <- fisk_logcdf(x, fit)
log_sf_vals <- fisk_logsf(x, fit)
...
# 5. Test Quantile / ISF
cat("Testing fisk_quantile/isf...\n")
p <- c(0.025, 0.5, 0.975)
qs <- fisk_quantile(p, fit)
isfs <- fisk_isf(1 - p, fit)
...
med <- fisk_median(fit)
int <- fisk_interval(0.95, fit)
m2 <- fisk_moment(2, fit)
cat(sprintf("  Median: %.4f\n", med))
cat(sprintf("  95%% Interval: [%.4f, %.4f]\n", int[1], int[2]))
stopifnot(abs(med - fisk_quantile(0.5, fit)) < 1e-7)
if (fit$shape > 2) {
...
ent <- fisk_entropy(fit)
exp_x <- fisk_expect(function(x) x, fit)
...
trunc_fit <- fisk_fit_truncated(trunc_data, lower = lower_bound, upper = upper_bound)
