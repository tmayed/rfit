# POC: Fitting 2-component mixture distributions to Danish fire insurance data
...
# 2. Fit all 2-component mixture combinations
cat("\nFitting all 2-component mixtures using fit_all_2m()...\n")
# This might take a few minutes as it fits many combinations
all_fits <- fit_all_2m(danish_data, criterion = "AIC")
...
output_dir <- "outputs"
output_file <- "danish_best_fit_2m_cdf"
...
# mixture_cdf is already available in mixture.R (sourced via rfit.R)
plot_cdf_comparison(
  ...
  title = sprintf("Empirical vs Fitted CDF (Best Mixture: %s)", best_name),
  x_label = "Loss Amount"
)
