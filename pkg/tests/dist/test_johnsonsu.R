...
cat("Testing johnsonsu_rand...\n")
test_data <- johnsonsu_rand(1000, fit_true)
stopifnot(length(test_data) == 1000)
cat(sprintf("  Mean of sample: %.4f\n", mean(test_data)))

# 2. Test Log-likelihood
cat("Testing johnsonsu_log_likelihood...\n")
ll <- johnsonsu_log_likelihood(test_data, 0.5, 1.5, 10, 2)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# 3. Test Fit
cat("Testing johnsonsu_fit...\n")
fit_hat <- johnsonsu_fit(test_data)
...
cat("Testing johnsonsu_fit_truncated...\n")
# Truncate data to [5, 15]
fit_t <- johnsonsu_fit_truncated(test_data, lower = 5, upper = 15)
cat(sprintf("  Truncated Log-likelihood: %.4f\n", fit_t$log_likelihood))
stopifnot(is.finite(fit_t$log_likelihood))

# 5. Test PDF / LogPDF
cat("Testing johnsonsu_pdf/logpdf...\n")
x <- seq(5, 15, length.out = 5)
dens <- johnsonsu_pdf(x, fit_hat)
log_dens <- johnsonsu_logpdf(x, fit_hat)
...
cat("Testing johnsonsu_cdf/sf/logcdf/logsf...\n")
cdf_vals <- johnsonsu_cdf(x, fit_hat)
sf_vals <- johnsonsu_sf(x, fit_hat)
...
stopifnot(all.equal(log(cdf_vals), johnsonsu_logcdf(x, fit_hat)))
stopifnot(all.equal(log(sf_vals), johnsonsu_logsf(x, fit_hat)))

# 7. Test Quantile / ISF
cat("Testing johnsonsu_quantile/isf...\n")
p <- c(0.25, 0.5, 0.75)
qs <- johnsonsu_quantile(p, fit_hat)
isfs <- johnsonsu_isf(1 - p, fit_hat)
cat(sprintf("  Quantiles: %s\n", paste(sprintf("%.4f", qs), collapse = ", ")))
stopifnot(all.equal(qs, isfs))
stopifnot(all.equal(johnsonsu_cdf(qs, fit_hat), p))

# 8. Test Mean / Var / Std / Skew / Kurtosis
cat("Testing moments/statistics...\n")
m <- johnsonsu_mean(fit_hat)
v <- johnsonsu_var(fit_hat)
s <- johnsonsu_std(fit_hat)
sk <- johnsonsu_skew(fit_hat)
kt <- johnsonsu_kurtosis(fit_hat)
...
med <- johnsonsu_median(fit_hat)
int <- johnsonsu_interval(0.95, fit_hat)
m1 <- johnsonsu_moment(1, fit_hat)
cat(sprintf("  Median: %.4f\n", med))
stopifnot(all.equal(med, johnsonsu_quantile(0.5, fit_hat)))
stopifnot(all.equal(m1, m, tolerance = 1e-5))

# 10. Test Entropy / Expect
cat("Testing entropy/expect...\n")
ent <- johnsonsu_entropy(fit_hat)
exp_x <- johnsonsu_expect(function(x) x, fit_hat)
