...
test_data <- kappa4_rand(1000, params)

# 1. Test log-likelihood
cat("Testing kappa4_log_likelihood...\n")
ll <- kappa4_log_likelihood(test_data, 10, 5, 0.1, 0.2)
cat(sprintf("  Log-likelihood: %.4f\n", ll))
stopifnot(is.finite(ll))

# 2. Test fit
cat("Testing kappa4_fit...\n")
fit <- kappa4_fit(test_data)
cat(sprintf("  xi: %.4f\n", fit$xi))
...
# 3. Test PDF / LogPDF
cat("Testing kappa4_pdf/logpdf...\n")
x <- seq(min(test_data), max(test_data), length.out = 10)
dens <- kappa4_pdf(x, fit)
log_dens <- kappa4_logpdf(x, fit)
...
cat("Testing kappa4_cdf/sf/logcdf/logsf...\n")
cdf_vals <- kappa4_cdf(x, fit)
sf_vals <- kappa4_sf(x, fit)
log_cdf_vals <- kappa4_logcdf(x, fit)
log_sf_vals <- kappa4_logsf(x, fit)
...
# 5. Test Quantile / ISF
cat("Testing kappa4_quantile/isf...\n")
p <- seq(0.1, 0.9, length.out = 5)
qs <- kappa4_quantile(p, fit)
isfs <- kappa4_isf(1 - p, fit)
...
med <- kappa4_median(fit)
int <- kappa4_interval(0.95, fit)
m2 <- kappa4_moment(2, fit)
cat(sprintf("  Median: %.4f\n", med))
cat(sprintf("  95%% Interval: [%.4f, %.4f]\n", int[1], int[2]))
stopifnot(all.equal(med, kappa4_quantile(0.5, fit)))
stopifnot(m2 > 0)

# 8. Test Entropy / Expect
cat("Testing entropy/expect...\n")
ent <- kappa4_entropy(fit)
exp_x <- kappa4_expect(function(x) x, fit)
...
q0 <- kappa4_quantile(0.5, fit0)
c0 <- kappa4_cdf(10, fit0)
